...
--- Apply attention styling to a tmux window by modifying window-status-current-format
--- This approach works with themed tmux configs that have hardcoded colors in format strings
---@param opts table|nil Options: { icon?: string, fg_color?: string, bg_color?: string, theme_bg?: string, theme_fg?: string, theme_accent?: string }
---@return table result { success: boolean, error: string|nil }
function M.set_attention_style(opts)
  if not M.is_inside_tmux() then
    return { success = false, error = "Not running inside tmux session" }
  end

  opts = opts or {}
  local icon = opts.icon or "󰚩 "
  local attention_bg = opts.bg_color or "#ff9500"
  local attention_fg = opts.fg_color or "#000000"
  -- Theme colors to replace (Dracula theme defaults)
  local theme_bg = opts.theme_bg or "#6272a4"
  local theme_fg = opts.theme_fg or "#f8f8f2"
  local theme_accent = opts.theme_accent or "#bd93f9"

  local window_index = M.get_current_window_index()
  if not window_index then
    return { success = false, error = "Failed to get current window index" }
  end

  -- Get the global window-status-current-format
  local orig_format = trim(vim.fn.system "tmux show-options -gv window-status-current-format")
  if orig_format == "" then
    return { success = false, error = "Failed to get window-status-current-format" }
  end

  -- Replace theme colors with attention colors and add icon
  local new_format = orig_format
    :gsub(theme_bg, attention_bg)
    :gsub(theme_fg, attention_fg)
    :gsub(theme_accent, attention_fg)
    :gsub("#I", "#I " .. icon)

  -- Apply the modified format to this window
  exec_tmux(
    string.format(
      "tmux set-window-option -t %s window-status-current-format %s",
      window_index,
      vim.fn.shellescape(new_format)
    )
  )
...
--- Clear attention styling from a tmux window
---@return table result { success: boolean, error: string|nil }
function M.clear_attention_style()
  if not M.is_inside_tmux() then
    return { success = false, error = "Not running inside tmux session" }
  end
  ...
  exec_tmux(string.format("tmux set-window-option -t %s -u window-status-current-format", window_index))
